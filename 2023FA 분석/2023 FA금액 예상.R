...
head(fa)
fa23 <- fa %>% filter(Start_year == 2023) # test data
str(fa)
names(fa)

fa_not23 <- fa %>% filter(Start_year != 2023) %>% filter(Start_year %in% seq(2012,2022,1)) # train data
nrow(fa_not23)
fa_not23$rank <- factor(fa_not23$rank, levels=c('S', 'A', 'B', 'C'))
# 등급은 정성평가 / 시장에서의 평가
contrasts(fa_not23$rank)

pairs.panels(fa_not23[c(6:9, 11, 14)])
...
final <- data.frame(Name=fa23$Name, X_money = money1, Real = c(3.1, 15, 2, 6.3, 1, 25.3, 10, 20, 12.5,0,
                                                               17.5, 3, 1.3, 11.5, 6.3, 0, 3.5, 16.3, 1.5, 
                                                               7.3, 4.5))
